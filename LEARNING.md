...
- [[TS-JS/函数重载方案]]——TS 四种运行时重载分派方式（typeof 链 / 字符串查表 / 嵌套对象 / canProcess 链），含 C++ 和 Rust 对照
- [[TS-JS/declare]]——`declare` 语法 + `.d.ts` 文件格式：模块扩展、值声明、全局扩展、在 Puerts 中的实际形态
...
myriad-mind-app（大衍决）项目方向：Tauri + React 做跨平台桌面应用。和 Puerts 不同——这是"前端的 TS"，跑在 WebView 里，不是 UE 脚本。

- [[Tauri-React/Tauri跨平台]]——Tauri 能跨平台共享什么、不共享什么；三平台三个 WebView 的坑；和 Electron 的"体积 vs 一致性"取舍；大衍决 monorepo 的"共享 core/ui + 各自外壳"策略 + `api.ts` 的能力检测降级
- [[Tauri-React/React数据流]]——从 JSX 到屏幕的单向数据链：组件=函数、props=参数、state=记忆、重渲染机制（Diff/Patch）、`&&` 条件渲染短路、响应式 vs 非响应式（灵魂一问）；大衍决 `useConfig` 的"持久层 → 桥接层 → 组件层"全链路实战
...
- 当前：完成第 1-6 章（`intro` ~ `move_semantics`），共 30 题
- 笔记：[[Rust/rustlings/notes/第1-6章总结]]——变量、函数、if 表达式、基本类型、Vec、所有权
...
4. **Neovim 学习路线**——渐进式替代 VSCode，和 Claude Code 协作。路线在 [[Terminal/Neovim]]；从 VSCode 迁移的对照表在 [[Terminal/VSCode对照]]；刷题工作流在 [[Terminal/Rustlings]]
5. **Homebrew**（macOS 包管理器）——之后装东西不用去官网下 `.dmg`，`brew install xxx` 一行搞定
6. **macOS 常用操作**——Spotlight、快捷键、多桌面、分屏——这些后面再补笔记
